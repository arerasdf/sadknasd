...
"""
부경대 중앙도서관 규정 전용 챗봇 페이지

✅ 특징
- 규정 전문이 비어 있으면 실행 전에 바로 막고 안내 메시지 출력
- 규정에 있는 내용은 최대한 찾아서 답변
- 정말 없을 때만 "해당 내용은 규정에 명시되어 있지 않습니다." 출력
- 전체 대화 히스토리를 유지하면서 대화형으로 질의응답
"""

import streamlit as st
from openai import OpenAI

st.title("실습 3 - 부경대 중앙도서관 규정 챗봇")

# -------------------------------------------------------------------
# 1. 여기에 '부경대학교 중앙도서관 이용 규정' 전문을 그대로 붙여 넣으세요.
#    (21_Lab 또는 21_Streamlit_02_advanced.pdf에 있는 규정 텍스트)
# -------------------------------------------------------------------
LIBRARY_RULES_TEXT = """
여기에 '부경대학교 중앙도서관 이용규정' 전문을 그대로 붙여 넣으세요.
...
※ 위 예시 문장은 전부 지우고, 과제 자료에 있는 실제 규정 텍스트로만 채우세요.
"""


# -------------------------------------------------------------------
# 2. 방어 코드: 규정 텍스트가 진짜로 들어갔는지 확인
# -------------------------------------------------------------------
def is_rules_text_valid(text: str) -> bool:
    """규정 텍스트가 실제로 채워졌는지 간단히 검사."""
    if not text:
        return False
    cleaned = text.strip()
    # 너무 짧으면(placeholder 상태) 규정이 아니라고 판단
    return len(cleaned) > 50 and "제1조" in cleaned


if not is_rules_text_valid(LIBRARY_RULES_TEXT):
    st.error(
        "⚠ 아직 도서관 규정 텍스트가 입력되지 않았습니다.\n\n"
        "코드 상단의 LIBRARY_RULES_TEXT 안에 "
        "'부경대학교 중앙도서관 이용 규정' 전문을 그대로 붙여 넣은 뒤 다시 실행하세요."
    )
    st.stop()
...
api_key_input = st.text_input(
    ...
    help="입력한 키는 이 브라우저 세션 내에서만 사용됩니다.",
)
...
# 5. 헬퍼: 모델 호출 함수
# -------------------------------------------------------------------
def ask_library_bot(question: str) -> str:
    """
    도서관 규정 + 대화 히스토리를 바탕으로 답변 생성.
    정말 관련 규정이 없을 때만 '명시되어 있지 않습니다' 문구 사용.
    """

    system_prompt = (
        "너는 '부경대학교 중앙도서관 이용 규정'만을 공식 근거로 삼는 도서관 안내 챗봇이다.\n"
        "아래에 규정 전문이 제공된다.\n\n"
        "1. 먼저 규정 텍스트에서 질문과 관련된 조항을 최대한 찾아라.\n"
        "2. 관련 조항이 있으면, 조항 번호/제목을 언급하고 이해하기 쉽게 요약해서 설명해라.\n"
        "3. 규정에 애매하게라도 연결될 수 있으면, 그 조항을 근거로 추론해서 답해라.\n"
        "4. 정말로 어떤 조항과도 연결되지 않는다면 그때에만\n"
        "   '해당 내용은 규정에 명시되어 있지 않습니다.'라고 답해라.\n"
        "5. 규정 원문을 그대로 복붙하지 말고, 필요한 부분만 짧게 인용하거나 요약해라.\n"
        "\n"
        "[부경대학교 중앙도서관 이용 규정 전문]\n"
        f"{국립부경대학교 도서관 규정
...
제4조(다른 규정의 개정) 본교 제 규정의 제명 및 내용 중 “부경대학교”는 “국립부경대학교”로 한다.}\n"
    ...
    # 공식 응답 형식에서 텍스트만 꺼내기
    # (output_text 헬퍼가 있다면 그걸 써도 되지만, 안전하게 직접 파싱)
    try:
        answer_text = response.output[0].content[0].text.value
    except Exception:
        # 혹시 형식이 바뀌었을 때를 대비한 안전 장치
...
# -------------------------------------------------------------------
st.info(
    "※ 이 챗봇은 **부경대학교 중앙도서관 이용 규정**에 기반하여 답변합니다.\n"
    "질문 예시: \n"
    "- '일반 자료실 이용 시간은 어떻게 되나요?'\n"
    "- '도서 연체 시 연체료는 어떻게 부과되나요?'\n"
    "- '한 번에 최대 몇 권까지 대출할 수 있나요?'"
)
...
# 7. 질문 입력 & 응답 처리
# -------------------------------------------------------------------
question = st.chat_input("도서관 이용 관련 궁금한 점을 입력하세요.")
...
if question:
    ...
    with st.chat_message("assistant"):
        with st.spinner("도서관 규정을 확인하는 중입니다..."):
            answer = ask_library_bot(question)
            st.markdown(answer)
